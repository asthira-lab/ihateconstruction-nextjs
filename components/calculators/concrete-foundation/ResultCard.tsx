...
// Renders the computed result of the concrete-foundation calculator.

import type { ConcreteFoundationResponse } from "@/features/calculators/concrete-foundation";

interface Props {
  data: ConcreteFoundationResponse;
}

export function ConcreteFoundationResultCard({ data }: Props) {
  ...
  return (
    <div className="space-y-4 rounded border border-black/10 bg-white p-5 dark:border-black dark:bg-black">
      ...
      <div className="space-y-1 border-t border-black/10 pt-3 text-xs text-black/60 dark:border-white/10 dark:text-white/60">
        <Row k="Footing" v={`${data.foundation.footingVolume.m3} m³`} />
        {data.input.stemWall.enabled ? (
          <Row k="Stem wall" v={`${data.foundation.stemWallVolume.m3} m³`} />
        ) : null}
        <Row k="Cubic metres (with wastage)" v={w.m3} />
...
export function concreteFoundationHeadline(data: ConcreteFoundationResponse): string {
