...
// Renders the computed result of the concrete-wall calculator.

import type { ConcreteWallResponse } from "@/features/calculators/concrete-wall";

interface Props {
  data: ConcreteWallResponse;
}

export function ConcreteWallResultCard({ data }: Props) {
  ...
  return (
    <div className="space-y-4 rounded border border-black/10 bg-white p-5 dark:border-black dark:bg-black">
      ...
      <div className="space-y-1 border-t border-black/10 pt-3 text-xs text-black/60 dark:border-white/10 dark:text-white/60">
        <Row k="Wall face area" v={`${data.wall.area.m2} m² / ${data.wall.area.ft2} ft²`} />
...
export function concreteWallHeadline(data: ConcreteWallResponse): string {
