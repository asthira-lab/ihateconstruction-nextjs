...
/**
 * ConcreteCalculatorForm — the client-side island.
 *
 ...
 * runs on the server. Validation here is UX polish; the server re-validates.
 */
...
function makeInitialState(
  standards: ConcreteStandardsResponse,
): FormState {
  const defaultPreset =
...
// ---- validation ---------------------------------------------------------
//
// Per-field, computed on every render. All numbers parsed as decimals; the
// backend's own range checks still run and surface as VALIDATION_FAILED, but
// obvious errors are caught here first.
...
function validateForm(state: FormState): FormErrors {
  ...
  if (!Number.isFinite(v) || v <= 0) {
    ...
    errors.volume = "Volume must be 10,000 or less.";
  }

  if (state.useCustom) {
    if (!MIX_RATIO_PATTERN.test(state.customMixRatio.trim())) {
      errors.customMixRatio = 'Use the format "a:b:c" — e.g. "1:1.5:3".';
    } else {
      const parts = state.customMixRatio.split(":").map(Number);
      if (parts.some((p) => !Number.isFinite(p) || p <= 0)) {
        errors.customMixRatio = "Every part of the ratio must be greater than 0.";
      }
    }

    const w = parseNum(state.customWastagePercent);
    if (!Number.isFinite(w) || w < 0) {
      errors.customWastagePercent = "Enter a number between 0 and 50.";
    } else if (w > 50) {
      errors.customWastagePercent = "Wastage must be 50% or less.";
    }

    const dw = parseNum(state.customDryToWetFactor);
    if (!Number.isFinite(dw) || dw <= 1) {
      errors.customDryToWetFactor = "Dry-to-wet factor must be greater than 1.";
    } else if (dw > 2) {
      errors.customDryToWetFactor = "Dry-to-wet factor above 2 is unrealistic.";
    }

    const cd = parseNum(state.customCementDensity.value);
    if (!Number.isFinite(cd) || cd <= 0) {
      errors.customCementDensity = "Enter a density greater than 0.";
    }

    const bw = parseNum(state.customCementBagWeight.value);
    if (!Number.isFinite(bw) || bw <= 0) {
      errors.customCementBagWeight = "Enter a bag weight greater than 0.";
    }
  }
    ...
    <div className="grid gap-8 md:grid-cols-[minmax(0,1fr)_minmax(0,20rem)]">
      <form onSubmit={onSubmit} className="space-y-8" noValidate>
        ...
        <fieldset className="space-y-2">
          <legend className="text-sm font-semibold">Concrete grade</legend>
          <select
          ...
          <label className="flex cursor-pointer items-center gap-2 pt-1 text-xs">
            ...
            />
            Customise this grade
          </label>
        </fieldset>

        {/* Volume input */}
        <fieldset className="space-y-4">
          <legend className="text-sm font-semibold">Concrete volume</legend>
          <LabeledRow label="Wet volume" error={errors.volume}>
            <QuantityInput
              value={state.volume}
          ...
          <p className="text-xs text-black/50 dark:text-white/50">
            Enter the finished (wet) volume of concrete you need to pour.
          </p>
        </fieldset>

        {/* Output unit selectors */}
        <fieldset className="space-y-3">
          <legend className="text-sm font-semibold">Output units</legend>
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <UnitSelect
              ...
              onChange={(v) =>
                set("outputAggregate", v as AggregateOutputUnit)
              }
        ...
        {state.useCustom ? (
          <fieldset className="space-y-4 rounded border border-black/10 p-4 dark:border-white/10">
            ...
            <LabeledRow label="Mix ratio (c : s : a)" error={errors.customMixRatio}>
              <input
            ...
            <LabeledRow label="Wastage" error={errors.customWastagePercent}>
              <NumberSliderInput
            ...
            <LabeledRow
              label="Dry-to-wet factor"
              error={errors.customDryToWetFactor}
            >
              <NumberSliderInput
                ...
                step={0.01}
                decimals={2}
                ariaLabel="Dry-to-wet factor"
              />
            </LabeledRow>

            <LabeledRow
              label="Cement density"
              error={errors.customCementDensity}
            >
              <QuantityInput
                ...
                step={10}
                decimals={0}
                ariaLabel="Cement bulk density"
              />
            </LabeledRow>

            <LabeledRow
              label="Cement bag weight"
              error={errors.customCementBagWeight}
            >
              <QuantityInput
