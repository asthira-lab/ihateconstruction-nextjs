// /calculators/concrete-slab-calculator — Server Component. Locale-aware shell.
// Human-readable article lives as MDX in content/concrete-slab/<locale>.mdx.
...
import { getConcreteSlabToc } from "@/features/calculators/concrete-slab/toc";
import { ConcreteSlabCalculatorForm } from "./ConcreteSlabCalculatorForm";
import { getDictionaryFor } from "@/app/[lang]/dictionaries";
import { isLocale, locales, defaultLocale } from "@/app/i18n-config";

import ConcreteSlabEn from "@/content/concrete-slab/en.mdx";
import ConcreteSlabHi from "@/content/concrete-slab/hi.mdx";
import ConcreteSlabEs from "@/content/concrete-slab/es.mdx";
import ConcreteSlabFr from "@/content/concrete-slab/fr.mdx";
import ConcreteSlabDe from "@/content/concrete-slab/de.mdx";
import ConcreteSlabAr from "@/content/concrete-slab/ar.mdx";

const CONCRETE_SLAB_ARTICLES: Record<string, typeof ConcreteSlabEn> = {
  en: ConcreteSlabEn,
  hi: ConcreteSlabHi,
  es: ConcreteSlabEs,
  fr: ConcreteSlabFr,
...
const PAGE_PATH = "/calculators/concrete-slab-calculator";
const OG_IMG = `${siteUrl}/concrete-slab-calculator-og.png`;
...
export async function generateMetadata({
  ...
  const t = (await getDictionaryFor(locale)).calculators["concrete-slab"].meta;
  ...
  return {
    ...
    keywords: [
      ...
      "80 lb bags of concrete for slab",
      "concrete slab depth",
    ],
    openGraph: {
      type: "website",
...
export default async function ConcreteSlabCalculatorPage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  ...
  const t = dict.calculators["concrete-slab"];
  const c = dict.calculators.common;
  const art = t.article;
  const Article = CONCRETE_SLAB_ARTICLES[locale] ?? ConcreteSlabEn;
  const toc = getConcreteSlabToc(locale);

  const others = CALCULATORS.filter((e) => e.slug !== "concrete-slab-calculator" && e.status === "live");
        ...
        <Container className="py-12">
          ...
          <div className="mt-10">
            <CalculatorPageShell
              calculator={
                <>
                  <ConcreteSlabCalculatorForm t={t.form} common={dict.common} cCommon={c} />
            ...
            >
              ...
              <CalculatorAd slotKey="concreteSlabInContent" />
          ...
          <section className="mt-12 border-t border-black/10 pt-10 dark:border-white/10">
            ...
            <CalculatorGrid locale={locale} comingLabel={dict.common.coming} filter={(e) => e.slug !== "concrete-slab-calculator"} />
          </section>
        </Container>
      </main>
      <SiteFooter />
