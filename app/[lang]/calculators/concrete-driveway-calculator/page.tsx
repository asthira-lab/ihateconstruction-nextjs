// /calculators/concrete-driveway-calculator — Server Component. Locale-aware shell.
// Human-readable article lives as MDX in content/concrete-driveway/<locale>.mdx.
...
import { getConcreteDrivewayToc } from "@/features/calculators/concrete-driveway/toc";
import { ConcreteDrivewayCalculatorForm } from "./ConcreteDrivewayCalculatorForm";
import { getDictionaryFor } from "@/app/[lang]/dictionaries";
import { isLocale, locales, defaultLocale } from "@/app/i18n-config";

import ConcreteDrivewayEn from "@/content/concrete-driveway/en.mdx";
import ConcreteDrivewayHi from "@/content/concrete-driveway/hi.mdx";
import ConcreteDrivewayEs from "@/content/concrete-driveway/es.mdx";
import ConcreteDrivewayFr from "@/content/concrete-driveway/fr.mdx";
import ConcreteDrivewayDe from "@/content/concrete-driveway/de.mdx";
import ConcreteDrivewayAr from "@/content/concrete-driveway/ar.mdx";

const CONCRETE_DRIVEWAY_ARTICLES: Record<string, typeof ConcreteDrivewayEn> = {
  en: ConcreteDrivewayEn,
  hi: ConcreteDrivewayHi,
  es: ConcreteDrivewayEs,
  fr: ConcreteDrivewayFr,
  de: ConcreteDrivewayDe,
  ar: ConcreteDrivewayAr,
};

const PAGE_PATH = "/calculators/concrete-driveway-calculator";
const OG_IMG = `${siteUrl}/concrete-driveway-calculator-og.png`;
...
export async function generateMetadata({
  ...
  const t = (await getDictionaryFor(locale)).calculators["concrete-driveway"].meta;
  ...
  return {
    ...
    keywords: [
      ...
      "driveway concrete yards",
      "concrete driveway thickness",
      "driveway reinforcement calculator",
      "wire mesh driveway",
      "rebar driveway spacing",
    ],
    openGraph: {
      type: "website",
...
export default async function ConcreteDrivewayCalculatorPage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  ...
  const t = dict.calculators["concrete-driveway"];
  const c = dict.calculators.common;
  const art = t.article;
  const Article = CONCRETE_DRIVEWAY_ARTICLES[locale] ?? ConcreteDrivewayEn;
  const toc = getConcreteDrivewayToc(locale);

  const others = CALCULATORS.filter((e) => e.slug !== "concrete-driveway-calculator" && e.status === "live");
        ...
        <Container className="py-12">
          ...
          <div className="mt-10">
            <CalculatorPageShell
              calculator={
                <>
                  <ConcreteDrivewayCalculatorForm t={t.form} common={dict.common} cCommon={c} />
            ...
            >
              ...
              {/* TODO: pick an existing slot key from app/lib/ads.ts or add a new one. */}
              <CalculatorAd slotKey="concreteDrivewayInContent" />
          ...
          <section className="mt-12 border-t border-black/10 pt-10 dark:border-white/10">
            ...
            <CalculatorGrid locale={locale} comingLabel={dict.common.coming} filter={(e) => e.slug !== "concrete-driveway-calculator"} />
          </section>
        </Container>
      </main>
      <SiteFooter />
