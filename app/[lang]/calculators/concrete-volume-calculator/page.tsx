// /calculators/concrete-volume-calculator — Server Component. Locale-aware shell.
// Human-readable article lives as MDX in content/concrete-volume/<locale>.mdx.
...
import { getConcreteVolumeToc } from "@/features/calculators/concrete-volume/toc";
import { ConcreteVolumeCalculatorForm } from "./ConcreteVolumeCalculatorForm";
import { getDictionaryFor } from "@/app/[lang]/dictionaries";
import { isLocale, locales, defaultLocale } from "@/app/i18n-config";

import ConcreteVolumeEn from "@/content/concrete-volume/en.mdx";
import ConcreteVolumeHi from "@/content/concrete-volume/hi.mdx";
import ConcreteVolumeEs from "@/content/concrete-volume/es.mdx";
import ConcreteVolumeFr from "@/content/concrete-volume/fr.mdx";
import ConcreteVolumeDe from "@/content/concrete-volume/de.mdx";
import ConcreteVolumeAr from "@/content/concrete-volume/ar.mdx";

const CONCRETE_VOLUME_ARTICLES: Record<string, typeof ConcreteVolumeEn> = {
  en: ConcreteVolumeEn,
  hi: ConcreteVolumeHi,
  es: ConcreteVolumeEs,
  fr: ConcreteVolumeFr,
...
const PAGE_PATH = "/calculators/concrete-volume-calculator";
const OG_IMG = `${siteUrl}/concrete-volume-calculator-og.png`;
...
export async function generateMetadata({
  ...
  const t = (await getDictionaryFor(locale)).calculators["concrete-volume"].meta;
  ...
  return {
    ...
    keywords: [
      ...
      "concrete volume for footing",
      "concrete volume for post hole",
    ],
    openGraph: {
      type: "website",
...
export default async function ConcreteVolumeCalculatorPage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  ...
  const t = dict.calculators["concrete-volume"];
  const c = dict.calculators.common;
  const art = t.article;
  const Article = CONCRETE_VOLUME_ARTICLES[locale] ?? ConcreteVolumeEn;
  const toc = getConcreteVolumeToc(locale);

  const others = CALCULATORS.filter((e) => e.slug !== "concrete-volume-calculator" && e.status === "live");
        ...
        <Container className="py-12">
          ...
          <div className="mt-10">
            <CalculatorPageShell
              calculator={
                <>
                  <ConcreteVolumeCalculatorForm t={t.form} common={dict.common} cCommon={c} />
            ...
            >
              ...
              <CalculatorAd slotKey="concreteVolumeInContent" />
          ...
          <section className="mt-12 border-t border-black/10 pt-10 dark:border-white/10">
            ...
            <CalculatorGrid locale={locale} comingLabel={dict.common.coming} filter={(e) => e.slug !== "concrete-volume-calculator"} />
          </section>
        </Container>
      </main>
      <SiteFooter />
