...
// Client-side form for the concrete-wall calculator. Owns the input state,
...
import { ConcreteWallResultCard } from "@/components/calculators/concrete-wall/ResultCard";
...
import type {
  ...
  ConcreteWallRequest,
  LengthQuantity,
} from "@/features/calculators/concrete-wall";
import { lengthToMeters } from "@/features/calculators/concrete-wall/units";
import { submitConcreteWallCalculation } from "./actions";
import type { Dictionary } from "@/app/[lang]/dictionaries";

const LENGTH_UNITS = ["m", "ft", "cm", "mm", "in"] as const;

type ConcreteWallFormDict = Dictionary["calculators"]["concrete-wall"]["form"];
...
function validateForm(state: FormState, e: ConcreteWallFormDict["errors"]): FormErrors {
  ...
  else if (hgtM > 30) errors.height = e.hgtMax;
...
function toRequest(state: FormState): ConcreteWallRequest {
  return {
    length: state.length,
    height: state.height,
...
interface ConcreteWallCalculatorFormProps {
  t: ConcreteWallFormDict;
  common: Dictionary["common"];
  cCommon: Dictionary["calculators"]["common"];
}

export function ConcreteWallCalculatorForm({ t, common, cCommon }: ConcreteWallCalculatorFormProps) {
  ...
  const [result, setResult] = useState<ConcreteWallActionResult | null>(null);
  const [savedRequest, setSavedRequest] = useState<ConcreteWallRequest | null>(null);
  const [isPending, startTransition] = useTransition();

  const run = (req: ConcreteWallRequest) =>
    startTransition(async () => {
      const res = await submitConcreteWallCalculation(req);
      setResult(res);
  ...
  return (
    <div className="space-y-4">
      ...
      <div aria-live="polite">
        {result?.ok ? (
          <>
            <ConcreteWallResultCard data={result.data} />
            <div className="mt-3 flex justify-end">
              <SaveToProjectButton
                calculator="concrete-wall"
