// /calculators/concrete-foundation-calculator — Server Component. Locale-aware shell.
// Human-readable article lives as MDX in content/concrete-foundation/<locale>.mdx.
...
import { getConcreteFoundationToc } from "@/features/calculators/concrete-foundation/toc";
import { ConcreteFoundationCalculatorForm } from "./ConcreteFoundationCalculatorForm";
import { getDictionaryFor } from "@/app/[lang]/dictionaries";
import { isLocale, locales, defaultLocale } from "@/app/i18n-config";

import ConcreteFoundationEn from "@/content/concrete-foundation/en.mdx";
import ConcreteFoundationHi from "@/content/concrete-foundation/hi.mdx";
import ConcreteFoundationEs from "@/content/concrete-foundation/es.mdx";
import ConcreteFoundationFr from "@/content/concrete-foundation/fr.mdx";
import ConcreteFoundationDe from "@/content/concrete-foundation/de.mdx";
import ConcreteFoundationAr from "@/content/concrete-foundation/ar.mdx";

const CONCRETE_FOUNDATION_ARTICLES: Record<string, typeof ConcreteFoundationEn> = {
  en: ConcreteFoundationEn,
  hi: ConcreteFoundationHi,
  es: ConcreteFoundationEs,
  fr: ConcreteFoundationFr,
  de: ConcreteFoundationDe,
  ar: ConcreteFoundationAr,
};

const PAGE_PATH = "/calculators/concrete-foundation-calculator";
const OG_IMG = `${siteUrl}/concrete-foundation-calculator-og.png`;
...
export async function generateMetadata({
  ...
  const t = (await getDictionaryFor(locale)).calculators["concrete-foundation"].meta;
...
export default async function ConcreteFoundationCalculatorPage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  ...
  const t = dict.calculators["concrete-foundation"];
  const c = dict.calculators.common;
  const art = t.article;
  const Article = CONCRETE_FOUNDATION_ARTICLES[locale] ?? ConcreteFoundationEn;
  const toc = getConcreteFoundationToc(locale);

  const others = CALCULATORS.filter((e) => e.slug !== "concrete-foundation-calculator" && e.status === "live");
        ...
        <Container className="py-12">
          ...
          <div className="mt-10">
            <CalculatorPageShell
              calculator={
                <>
                  <ConcreteFoundationCalculatorForm t={t.form} common={dict.common} cCommon={c} />
            ...
            >
              ...
              <CalculatorAd slotKey="concreteFoundationInContent" />
          ...
          <section className="mt-12 border-t border-black/10 pt-10 dark:border-white/10">
            ...
            <CalculatorGrid locale={locale} comingLabel={dict.common.coming} filter={(e) => e.slug !== "concrete-foundation-calculator"} />
          </section>
        </Container>
      </main>
      <SiteFooter />
