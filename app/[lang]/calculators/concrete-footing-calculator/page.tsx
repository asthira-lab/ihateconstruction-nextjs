// /calculators/concrete-footing-calculator — Server Component. Locale-aware shell.
// Human-readable article lives as MDX in content/concrete-footing/<locale>.mdx.
...
import { getConcreteFootingToc } from "@/features/calculators/concrete-footing/toc";
import { ConcreteFootingCalculatorForm } from "./ConcreteFootingCalculatorForm";
import { getDictionaryFor } from "@/app/[lang]/dictionaries";
import { isLocale, locales, defaultLocale } from "@/app/i18n-config";

import ConcreteFootingEn from "@/content/concrete-footing/en.mdx";
import ConcreteFootingHi from "@/content/concrete-footing/hi.mdx";
import ConcreteFootingEs from "@/content/concrete-footing/es.mdx";
import ConcreteFootingFr from "@/content/concrete-footing/fr.mdx";
import ConcreteFootingDe from "@/content/concrete-footing/de.mdx";
import ConcreteFootingAr from "@/content/concrete-footing/ar.mdx";

const CONCRETE_FOOTING_ARTICLES: Record<string, typeof ConcreteFootingEn> = {
  en: ConcreteFootingEn,
  hi: ConcreteFootingHi,
  es: ConcreteFootingEs,
  fr: ConcreteFootingFr,
  de: ConcreteFootingDe,
  ar: ConcreteFootingAr,
};

const PAGE_PATH = "/calculators/concrete-footing-calculator";
const OG_IMG = `${siteUrl}/concrete-footing-calculator-og.png`;
...
export async function generateMetadata({
  ...
  const t = (await getDictionaryFor(locale)).calculators["concrete-footing"].meta;
  ...
  return {
    ...
    keywords: [
      ...
      "continuous wall footing concrete",
      "spread footing concrete",
      "pier footing concrete",
      "cubic yards of concrete for footing",
    ],
    openGraph: {
      type: "website",
...
export default async function ConcreteFootingCalculatorPage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  ...
  const t = dict.calculators["concrete-footing"];
  const c = dict.calculators.common;
  const art = t.article;
  const Article = CONCRETE_FOOTING_ARTICLES[locale] ?? ConcreteFootingEn;
  const toc = getConcreteFootingToc(locale);

  const others = CALCULATORS.filter((e) => e.slug !== "concrete-footing-calculator" && e.status === "live");
        ...
        <Container className="py-12">
          ...
          <div className="mt-10">
            <CalculatorPageShell
              calculator={
                <>
                  <ConcreteFootingCalculatorForm t={t.form} common={dict.common} cCommon={c} />
            ...
            >
              ...
              <CalculatorAd slotKey="concreteFootingInContent" />
          ...
          <section className="mt-12 border-t border-black/10 pt-10 dark:border-white/10">
            ...
            <CalculatorGrid locale={locale} comingLabel={dict.common.coming} filter={(e) => e.slug !== "concrete-footing-calculator"} />
          </section>
        </Container>
      </main>
      <SiteFooter />
